...
exports.run = function (options, callback) {
                ...
                exists(path.join(p, 'build.json'), modStack.add(function (yes) {
                    ...
                    if (yes) {
                        ...
                        if (stat.isDirectory()) {
                            ...
                            entry = checkYUIModule(p);
                            if (entry && (!options.modules || has(options.modules, entry.name))) {
                                relative = workingdir.replace(startdir, '');
                                entry.path = path.join(relative, mod);
                                mods.push(entry);
                            }
                        }
                    }
    ...
    modStack.done(function () {
        ...
        var stack = new Stack(),
            errors = [],
            run = function () {
                var mod = mods.pop(), child, builds, builder;
