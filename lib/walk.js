...
exports.run = function (options, callback) {
    ...
    if (options['include-dirs']) {
        includeDirs = options['include-dirs'].split(',');
        Object.keys(includeDirs).forEach(function (k) {
            // normalizing the dirname
            includeDirs[k] = path.join('./', includeDirs[k]);
        });
    }

    if (options['exclude-dirs']) {
        excludeDirs = options['exclude-dirs'].split(',');
        Object.keys(excludeDirs).forEach(function (k) {
            // normalizing the dirname
            excludeDirs[k] = path.join('./', excludeDirs[k]);
        });
    }
        ...
        fs.readdir(workingdir, modStack.add(function (err, dirs) {
            dirs.forEach(function (mod) {
                var p = path.join(workingdir, mod),
                    isDir = (includeDirs || excludeDirs) && fs.statSync(p).isDirectory(),
                    relativedir = path.join('./', p.replace(startdir, ''));
                if (isDir && excludeDirs &&  -1 !== excludeDirs.indexOf(relativedir)) {
                    log.warn('skipping folder `' + relativedir + '`, it is part of `exclude-dirs` option.');
                    return;
                }
                if (isDir && includeDirs && -1 === includeDirs.indexOf(relativedir)) {
                    log.warn('skipping folder `' + relativedir + '`, it is NOT part of `include-dirs` option.');
                    return;
                }
