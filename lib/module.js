...
var copyFile = function(from, to, callback) {
    var fromS = fs.createReadStream(from),
        toS = fs.createWriteStream(to);

    fromS.pipe(toS);
    fromS.once('end', callback);
};

var buildNPM = function(json, callback) {
    ...
    if (exists(path.join(shifter.cwd(), 'package.json'))) {
        ...
        fs.writeFileSync(path.join(buildDir, 'package.json'), JSON.stringify(packageJson, null, 4), 'utf8');
        writeShim();
    }
};
...
var _rollup = function (mod, name, options, callback) {
    ...
    mod.files.forEach(function (file) {
        files.push(path.join(shifter.cwd(), '../../build/', file, file + '-debug.js'));
    });
